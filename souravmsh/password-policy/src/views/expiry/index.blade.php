@extends('password-policy::layout')

@section('content') 
<header class="header mt-3">
    <p class="h4"><strong>{{ $page ?? '' }}</strong></p>
    <div class="pull-right" style="margin-top: 0px">
        <a href="{{  route('password-policy.expiry.create')  }}" class="btn btn-sm btn-primary" data-modal="ajaxifyModal">
            <i class="fa fa-plus"></i> Add User Expiration
        </a>
        <a href="{{  route('password-policy.pretend.show')  }}" class="btn btn-sm btn-danger" data-modal="ajaxifyModal">
    ...
    <form action="#" method="get">
        <div class="row">
            ...
            <div class="col-sm-4">
                ...
                <a href="{{ route('password-policy.expiry') }}" class="btn btn-sm btn-danger">Clear</a>
            </div>
        </div>
    </form>
</section>
                ...
                @if($result->count())
                    @foreach($result as $item)
                        <tr>
                            ...
                            <td style="width: 10%">
                                <a class="btn btn-primary btn-sm" data-modal="ajaxifyModal" href="{{ route('password-policy.expiry.edit', $item->id) }}"> <i class="fa fa-edit"></i> Edit</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
