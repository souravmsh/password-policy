...
namespace Souravmsh\PasswordPolicy\Http\Traits;
...
| \Modules\PasswordPolicy\Http\Middleware\PasswordExpiryCheck::class,
|
...
trait PasswordPolicy
{
    ...
    private $passwordLoginIsExpiredRoute = 'password-policy.password';
    private $passwordLoginIsExpiredRouteAllowed = ['password-policy.password', 'password-policy.password.reset'];
    ...
    {
        $this->passwordCacheMinutes = config('password-policy.cache_minutes');
        $this->passwordDBOldPasswordCheck = config('password-policy.check_old_password');
    }
    ...
    {   
        if (auth()->hasUser())
        {
            ...
            $data = cache()->remember('passwordLoginIsExpired', $this->passwordCacheMinutes, function () {
                return \DB::table('password_policy_expiry')
            ...
            if (empty($data))
            {  
                \DB::table('password_policy_expiry')->insert([
    ...
    */ 
    ...
    {  
        return cache()->remember('passwordDBRules', $this->passwordCacheMinutes, function () {
            return \DB::table('password_policy_rules')
                ->where('value', '!=', 'false')
    ...
    {       
        return cache()->remember('passwordDBCheckList', $this->passwordCacheMinutes, function () {
            return \DB::table('password_policy_checklist')
                ->where('status', 1)
                ->pluck('password')
                ->toArray();
        }); 
